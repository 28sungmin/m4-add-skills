-- ═══════════════════════════════════════════════════════════════════
-- MIMIC-IV → QUIQ Table (BigQuery version, v6 260529)
--
-- 원본: 01_create_macros.sql + 02_mimic_to_quiq_v6_260529.sql (DuckDB)
-- 참조: 품질관리 프로그램 활용 가이드라인 제4권 §1.5~§1.6
--
-- BigQuery 변환 사항:
--   CREATE MACRO → CREATE TEMP FUNCTION
--   TRY_STRPTIME → SAFE.PARSE_TIMESTAMP / SAFE.PARSE_DATE
--   TRY_CAST AS DOUBLE → SAFE_CAST AS FLOAT64
--   VARCHAR → STRING, 큰따옴표 예약어 → 백틱
--   hosp./icu./ed. → physionet-data.mimiciv_3_1_{hosp,icu}/mimiciv_ed.
--   UNPIVOT (DuckDB 문법) → UNPIVOT (BigQuery 문법)
--   LIMIT ALL 제거
--   실제 BigQuery 스키마 컬럼명 수정 (reason_for_no_barcode 등)
--   procedureevents.secondaryordercategoryname 제거 (BigQuery에 없음)
-- ═══════════════════════════════════════════════════════════════════

-- ═══════════════════════════════════════════════════════════════════
-- MIMIC-IV Synthetic Data → QUIQ Table (SQL version, v5 260527)
--
-- 참조: MIMICIV_to_QUIQ_colab_3.py, QUIQ_MIMICIV.sql
--       품질관리 프로그램 활용 가이드라인 제4권 §1.5~§1.6
--
-- 테이블별 변환 전략
--   ① Wide  테이블 (admissions, patients 등) : UNPIVOT (컬럼 → 행)
--   ② Code  테이블 (diagnoses_icd, procedures_icd)
--           : d_icd_* JOIN → 원본 1행 → 2 QUIQ 행 (icd_code + long_title)
--   ③ Event 테이블 (labevents)
--           : d_labitems JOIN → Variable_name = label_fluid
--             원본 1행 → 최대 7 QUIQ 행 (측정값 + 부가 컬럼)
--             subject_id 기준 정렬 (같은 환자 행이 인접하도록)
--   ④ Event 테이블 (chartevents)
--           : d_items JOIN → Variable_name = label
--             원본 1행 → 1 QUIQ 행
--
-- < 실행 방법 >
-- 해당 프로젝트 폴더로 이동
-- ~/.local/bin/duckdb mimic_synthetic.duckdb < queries/01_create_macros.sql
-- ~/.local/bin/duckdb mimic_synthetic.duckdb < queries/02_mimic_to_quiq_v5_260527.sql
-- ═══════════════════════════════════════════════════════════════════

-- ── BigQuery TEMP FUNCTION (01_create_macros.sql 기반) ─────────────
...
CREATE TEMP FUNCTION _map1(tbl STRING, col STRING)
RETURNS STRING AS (
    CASE
        WHEN col IN ('admittime','dischtime','deathtime','edregtime','edouttime','dod',
                     'intime','outtime','scheduletime','storetime','chartdate','charttime',
                     'storedate','starttime','stoptime','entertime','verifiedtime',
                     'expirationdate','ordertime','transfertime','endtime','comments_date')
            THEN 'date'
        WHEN col IN ('icd_code','icd9_code','icd10_code','hcpcs_cd','drg_code')
            THEN 'medical_code'
        WHEN col IN ('medication','drug','name')
             AND (tbl LIKE '%emar%' OR tbl LIKE '%pharmacy%' OR tbl LIKE '%prescriptions%'
                  OR tbl LIKE '%medrecon%' OR tbl LIKE '%pyxis%')
            THEN 'prescription'
        WHEN col IN ('dose_val_rx','dose_due','dose_given','doses_per_24_hrs',
                     'duration','frequency','route','fill_quantity','prod_strength',
                     'form_rx','form_val_disp','basal_rate','one_hr_max')
            THEN 'prescription'
        WHEN (tbl LIKE '%procedure%' OR col = 'proc_type')
             AND col NOT IN ('value','valuenum','valueuom','unit')
            THEN 'procedure'
        WHEN tbl LIKE '%lab%' AND col IN ('value','valuenum')
            THEN 'event'
        WHEN (tbl LIKE '%chart%' OR tbl LIKE '%vital%' OR tbl LIKE '%triage%')
             AND col IN ('value','valuenum','temperature','heartrate','resprate',
                         'o2sat','sbp','dbp','pain','acuity')
            THEN 'event'
        WHEN (tbl LIKE '%input%' OR tbl LIKE '%output%' OR tbl LIKE '%ingredient%')
             AND col IN ('value','amount','rate','totalamount')
            THEN 'event'
        WHEN col IN ('icd_title','description','long_title','long_description',
                     'chiefcomplaint','spec_type_desc')
            THEN 'diagnosis'
        ELSE ''
    END
);

CREATE TEMP FUNCTION _map2(tbl STRING, col STRING)
RETURNS STRING AS (
    CASE
        WHEN col IN ('medication','drug','name')
             AND (tbl LIKE '%emar%' OR tbl LIKE '%pharmacy%' OR tbl LIKE '%prescriptions%'
...
t_admissions AS (
    SELECT ROW_NUMBER() OVER (ORDER BY (SELECT 1)) - 1 AS _pk,
        subject_id, hadm_id,
...
u_admissions AS (
    ...
    UNPIVOT (raw_val FOR Variable_name IN (admittime, dischtime, deathtime, admission_type, admission_location, discharge_location, insurance, `language`, marital_status, race, edregtime, edouttime, hospital_expire_flag))
),
...
-- ── `physionet-data`.mimiciv_3_1_hosp.emar_detail ─────────────────────────────────────────────
t_emar_detail AS (
    SELECT ROW_NUMBER() OVER (ORDER BY (SELECT 1)) - 1 AS _pk,
        subject_id,
        ...
        COALESCE(CAST(reason_for_no_barcode            AS STRING), '') AS reason_for_no_barcode,
...
u_emar_detail AS (
    ...
    UNPIVOT (raw_val FOR Variable_name IN (emar_seq, parent_field_ordinal, administration_type, barcode_type, reason_for_no_barcode, complete_dose_not_given, dose_due, dose_given, will_remainder_be_given, product_amount_given, product_code, product_description, product_description_other, prior_infusion_rate, infusion_rate, infusion_rate_adjustment, infusion_rate_adjustment_amount, `route`, infusion_complete, completion_interval, new_iv_bag_hung, continued_infusion_in_other_location, restart_interval, side, `site`, non_formulary_visual_verification))
),
...
u_micro AS (
    SELECT * FROM t_micro
    UNPIVOT (raw_val FOR Variable_name IN (spec_type_desc, test_seq, storedate, storetime, test_name, org_name, isolate_num, quantity, ab_name, dilution_text, dilution_comparison, dilution_value, interpretation, comments))
),
...
u_pharmacy AS (
    ...
    UNPIVOT (raw_val FOR Variable_name IN (starttime, stoptime, medication, proc_type, `status`, entertime, verifiedtime, `route`, frequency, disp_sched, infusion_type, sliding_scale, lockout_interval, basal_rate, one_hr_max, doses_per_24_hrs, duration, expiration_value, expirationdate, dispensation, fill_quantity))
),

-- ── `physionet-data`.mimiciv_3_1_hosp.poe ─────────────────────────────────────────────────────
t_poe AS (
    SELECT ROW_NUMBER() OVER (ORDER BY (SELECT 1)) - 1 AS _pk,
        subject_id, hadm_id,
...
-- ── `physionet-data`.mimiciv_3_1_hosp.poe_detail ──────────────────────────────────────────────
t_poe_detail AS (
    SELECT ROW_NUMBER() OVER (ORDER BY (SELECT 1)) - 1 AS _pk,
        subject_id,
        COALESCE(CAST(poe_seq     AS STRING), '') AS poe_seq,
...
u_prescriptions AS (
    ...
    UNPIVOT (raw_val FOR Variable_name IN (poe_seq, starttime, stoptime, drug_type, drug, formulary_drug_cd, gsn, ndc, prod_strength, form_rx, dose_val_rx, form_val_disp, doses_per_24_hrs, `route`))
),
...
--    q_labevents_all 내 ORDER BY + 로 같은 환자 행이 인접하도록 보장
-- ════════════════════════════════════════════════════════════════════

-- ── `physionet-data`.mimiciv_3_1_hosp.labevents (base) ────────────────────────────────────────
...
-- ── `physionet-data`.mimiciv_3_1_hosp.labevents (long: 7컬럼 전개, subject_id 정렬) ───────────
-- base의 컬럼들을 "행으로 분해"
--: DuckDB에서 ORDER BY 결과를 보장하기 위해 명시
q_labevents_all AS (
    -- ① value (메인 측정값)
...
-- ── `physionet-data`.mimiciv_3_1_icu.datetimeevents ───────────────────────────────────────────
...
-- ── `physionet-data`.mimiciv_3_1_icu.ingredientevents ─────────────────────────────────────────
...
-- ── `physionet-data`.mimiciv_ed.medrecon ──────────────────────────────────────────────────
t_medrecon AS (
    SELECT ROW_NUMBER() OVER (ORDER BY (SELECT 1)) - 1 AS _pk,
        subject_id,
        CAST(charttime AS STRING)                       AS _ev,
...
SELECT _pk, '', 'EMAR_DETAIL', Variable_name, NULL, raw_val,
       ...
       -- product_description: 약물명+농도+용량+제형이 혼재("Vancomycin 5 mg/mL 250 mL IV Bag")
       --   → drug / prescription_info 구분 불가 → Mapping_info 미부여(NULL)
       CASE Variable_name
...
SELECT _pk, '', 'MICROBIOLOGYEVENTS', Variable_name,
       ...
       -- dilution_text/comparison/value: "<=0.06" 형태의 MIC 판정 보조값
       --   → 비교연산자+수치 혼재, 순수 검사 결과값으로 분류 불가 → Mapping_info 미부여(NULL)
       CASE Variable_name
