...
def draw_diversity_box_plot(ax: Axes, results_df: pd.DataFrame):
    ax.clear()
    simpson_scores = results_df['Class_diversity (%)']  # fixed: was 'Class_Simpson_diversity'
...
def _calculate_and_plot_diversity(df: pd.DataFrame):
    ...
    df_grouped_idx = (
        df.groupby(['Original_table_name', 'Variable_name'])
        .count()
...
def get_class_diversity(quiq: pd.DataFrame):
...
if __name__ == '__main__':
    ...
    weighted_class_diversity = round(
        (df_summary['Total Number of Data'] * df_summary['Class_diversity (%)']).sum() / total_num, 2
    )

    with open(save_path + '/class_diversity_total.txt', 'w', encoding='utf-8') as f:
        f.write(f'Weighted Class Diversity (%) = {weighted_class_diversity}\n')
    ...
    with open(save_path + '/class_diversity_detail.txt', 'w', encoding='utf-8') as f:
        for current_identifier in df_summary['identifier']:
            ...
            for _, row in current_df.iterrows():
                f.write(f"Class : {row['Class']}  |  Count : {row['Count']}\n")
            f.write('\n')
