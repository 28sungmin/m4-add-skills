...
def draw_auroc_variable_plot(save_path, n, table_name, variable_name, y_true, y_pred, unique_labels):
    ...
    safe_table = re.sub(r'[\\/:*?"<>|]', ' ', table_name)
    safe_var = re.sub(r'[\\/:*?"<>|]', ' ', variable_name)
    fig.savefig(os.path.join(save_path, 'classification_plots', f'{n}_{safe_table}_{safe_var}.png'))
...
def get_classification(quiq: pd.DataFrame):
    ...
    for idx, target_df in df_grouped:
        ...
        f1score = round(f1_score(y_true, y_pred, average='macro', zero_division=0), 3)
    ...
    result_df = pd.DataFrame(
        results,
        columns=['Original_table_name', 'Variable_name', 'Accuracy', 'Precision', 'Recall', 'F1Score', 'AUROC']
    )
...
if __name__ == '__main__':
    ...
    os.makedirs(os.path.join(save_path, 'classification_plots'), exist_ok=True)

    with open(os.path.join(save_path, 'classification_total.txt'), 'w', encoding='utf-8') as f:
    ...
    result_df.to_csv(os.path.join(save_path, 'classification_summary.csv'), index=False)
