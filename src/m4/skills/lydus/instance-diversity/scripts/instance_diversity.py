...
def _calculation(instance_values):
    filtered_values = [v for v in instance_values if pd.notnull(v) and v != 0]
...
def _calculate_class_instance_counts(class_values, instance_values):
    ...
    for _, row in class_counts.iterrows():
        ...
        weighted_instance_diversities.append(instance_diversity * total_count if instance_diversity is not None else 0)
        weighted_simpson_diversities.append(simpson_diversity_score * total_count if simpson_diversity_score is not None else 0)
...
def get_instance_diversity(quiq: pd.DataFrame):
    ...
    df_filtered['ClassKey'] = list(zip(
        df_filtered['Original_table_name'],
        df_filtered['Variable_name'],
        df_filtered['Value']
    ))

    results_df, weighted_avg_instance_diversity, weighted_avg_simpson_diversity = \
        _calculate_class_instance_counts(df_filtered['ClassKey'], df_filtered['Patient_id'])
...
if __name__ == '__main__':
    ...
    results_df, weighted_avg_instance_diversity, weighted_avg_simpson_diversity = get_instance_diversity(quiq)

    with open(save_path + '/instance_diversity_total.txt', 'w', encoding='utf-8') as f:
        f.write(f'Weighted Instance Diversity (%) = {weighted_avg_instance_diversity}\n')
        f.write(f'Weighted Gini-Simpson Index (%) = {weighted_avg_simpson_diversity}\n')

    results_df.to_csv(save_path + '/instance_diversity_summary.csv', index=False)
